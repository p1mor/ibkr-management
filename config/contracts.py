...
if __name__ == "__main__":
    # Demo de uso
    print("\n" + "="*70)
    print("DEMO: ContractBuilder")
    print("="*70 + "\n")
    
    # Listar símbolos disponibles
    print("Símbolos pre-configurados:")
    for symbol in ContractBuilder.list_available_symbols():
        print(f"  - {symbol}")
    
    print("\n")
    
    # Información detallada de ES
    ContractBuilder.print_contract_info('ES')
    ...
    # Cálculos de ejemplo
    es_price = 5875.00
    print(f"Ejemplo de cálculos para ES a {es_price}:")
    print(f"  Valor 1 contrato: ${ContractBuilder.calculate_contract_value('ES', es_price, 1):,.2f}")
    print(f"  Valor 1 tick:     ${ContractBuilder.calculate_tick_value('ES', 1)}")
    ...
    es_contract = ContractBuilder.create_future_contract()
    print(f"  ES Futuro: {es_contract.symbol} {es_contract.secType} @ {es_contract.exchange}")
    
    nq_contract = ContractBuilder.create_future_contract(symbol='NQ')
    print(f"  NQ Futuro: {nq_contract.symbol} {nq_contract.secType} @ {nq_contract.exchange}")
    
    aapl_contract = ContractBuilder.create_stock_contract('AAPL')
    print(f"  AAPL Stock: {aapl_contract.symbol} {aapl_contract.secType} @ {aapl_contract.exchange}")
    
    forex_contract = ContractBuilder.create_forex_contract('EURUSD')
    print(f"  EUR/USD Forex: {forex_contract.symbol}/{forex_contract.currency} {forex_contract.secType} @ {forex_contract.exchange}")
